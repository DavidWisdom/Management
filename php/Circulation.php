...
function Borrowing($isGet)
{
    ...
    $sql = "INSERT INTO Borrowing
            (BookBarcode, RBarcode, StartTime, ExpiryTime, DisabledRenew, Operator, Stage)
            VALUES
            ('$bBarcode', '$rBarcode', '$startTime', '$expiryTime', False, '$operator' ,'$stage')";
...
function SendBack($isGet)
{
    ...
    $sql = "INSERT INTO SendBack
                (BookBarcode, RBarcode, ReturnTime, Operator, BRId, Stage)
                VALUES
                ('$bBarcode', '$rBarcode', '$time', '$operator', $brId, '$stage')";
...
function ReNew($isGet)
{
    ...
    $expiryTime = date('Y-m-d H:i:s', strtotime("{$expiryTime} +1 day"));
